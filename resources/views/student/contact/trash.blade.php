                            ...
                            <div class="col-12">
                                <div class="email-leftbar card">
                                    ...
                                    <div class="mail-list mt-4">
                                        ...
                                        <a href="{{url('student/contact/trash')}}" class="active">Trash <span class="ml-1">
                                                @if(count($contacts) > 0)
                                                    ({{count($contacts)}})
                                                @endif
                                            </span></a>
                                ...
                                <div class="email-rightbar mb-3">
                                    <div class="card">
                                        <div class="btn-toolbar p-3" role="toolbar">
                                            @if (count($contacts) > 0)
                                                <form action="{{url('student/contact/delete')}}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                ...
                                                @foreach($contacts as $contact)
                                                    <li class="{{ $contact->read == 0 ? 'unread' : ''}}">
                                                        <div class="col-mail col-mail-1">
                                                            ...
                                                            <a href="{{url('student/contact/read', $contact->id)}}" class="title">{{$contact->id_sender == Auth::user()->id ? 'me' : $contact->sender->firstname}}, {{$contact->id_reciever == Auth::user()->id ? 'me' : $contact->reciever->firstname}}</a>
                                                        </div>
