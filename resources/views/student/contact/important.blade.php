                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="email-leftbar card">
                                    <a href="{{url('/student/contact/add')}}" class="btn btn-danger rounded btn-custom btn-block waves-effect waves-light">Compose</a>
        
                                    <div class="mail-list mt-4">
                                        <a href="{{url('student/contact')}}">Inbox</a>
                                        <a href="{{url('student/contact/star')}}">Starred</a>
                                        <a href="{{url('student/contact/important')}}" class="active">Important <span class="ml-1">
                                                @if(count($contacts) > 0)
                                                    ({{count($contacts)}})
                                                @endif
                                            </span></a>
                                                        ...
                                                        <tr>
                                                            <td colspan="7" class="text-center">
                                                                <img src="{{asset('images/no_data.svg')}}" height="124" class="mt-3">
                                                                    ...
                                                                    <a href="{{url('/student/contact')}}" class="btn btn-primary waves-effect waves-light">Refresh</a>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                                ...
                                                @foreach($contacts as $contact)
                                                    <li class="{{ $contact->read == 0 ? 'unread' : ''}}">
                                                        <div class="col-mail col-mail-1">
                                                            ...
                                                            <a href="{{url('student/contact/read', $contact->id)}}" class="title">{{$contact->id_sender == Auth::user()->id ? 'me' : $contact->sender->firstname}}, {{$contact->id_reciever == Auth::user()->id ? 'me' : $contact->reciever->firstname}}</a><span class="star-toggle {{ $contact->star == 1 ? 'fas' : 'far'}} fa-star"></span>
                                                        </div>
                                                        <div class="col-mail col-mail-2">
                                                            <a href="{{url('student/contact/read', $contact->id)}}" class="subject">{{$first = explode(' ', $contact->message, 2)[0]}} – <span class="teaser">{{$teaser = explode(' ', $contact->message, 2)[1]}}</span></a>
